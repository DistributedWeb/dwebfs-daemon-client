...
class DaemonCommand extends Command {
  ...
  static drivePathArg (info) {
    return {
      name: 'path',
      description: 'The path to the drive (must be within ~/DWebFs)',
      parse: path => {
        if (!path) return constants.mountpoint
        path = p.resolve(path)
        if (path.startsWith(constants.networkPath)) throw new Error('DWebFs commands cannot be run on the Network directory.')
        if (path.startsWith(constants.mountpoint)) return path
        throw new Error(`Path ${path} is not contained within the DWebFs mountpoint`)
      },
      ...info
    }
  }
  ...
  async run () {
    try {
      ...
      console.error('Could not connect to the daemon. Is it running? Check with `dwebfs status`.')
      this.exit(1)
    }
  }
}
