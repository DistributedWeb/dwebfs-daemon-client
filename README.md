...
A Node client library and CLI tool for interacting with the DWebFs daemon.
...
## Usage
This module provides both programmatic and CLI access to the [DWebFs daemon](https://github.com/andrewosh/dwebfs-daemon). For info about how to use the CLI, take a look at README in the daemon repo.
...
If you're running the client and the daemon on the same machine, the endpoint/token can be read from a common location (by default, `~/.dwebfs`). If the arguments are not provided, then they will be read from this file (which is created by the daemon).

All DWebFs API methods are accessed through `client.drive`, and all FUSE methods through `client.fuse`.
...
### DWebFs
The client's DWebFs API is designed to mirror the methods in DWebFs as closely as possible.
...
Creates a DWebFs using the provided drive options (if one has not previously been created), then opens a session for that drive.

Options can include:
- `key`: The key of an existing DWebFs
...
Returns:
- `drive`: A remote DWebFs instance that can be used for subsequent drive-specific commands.
...
Each of the following is not a DWebFs method, but applies only to a single session.
...
#### DWebFs Methods
The client currently only supports a subset of the DWebFs API. We're actively working on extending this (targeting complete parity)! Each method's options mirror those in the [dwebfs module](https://github.com/distributedweb/dwebfs).

Each method returns a Promise, but can optionally take a callback (to more accurately reflect the DWebFs API).

Method arguments take the same form as those in DWebFs. The following methods are supported as of now:
...
- `mnt`: The mountpoint of the drive (currently enforced to be `~/DWebFs` if it's the root drive, and a subdirectory within `~/DWebFs` otherwise.
- `opts`: DWebFs mount options (identical to those in DWebFs).
